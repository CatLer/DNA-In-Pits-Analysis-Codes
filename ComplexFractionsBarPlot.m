function [] = ComplexFractionsBarPlot(varargin)
...
switch opt % 3 different scenarios
    ...
    case 'Temperature'   % plot # of bound complexes VS T
        ...
        for i=1:numel(TPU)
            ...
            ComplexFrac(i)=mean(BindingNum_Prime(Temperatures_Prime==TPU(i)));
        end
        %..................................................................
        if numel(TPU)>1 % if more than 1 temperature
            % power fit # of bound complexes VS temperature
            [f,g,o]=fit(TPU(:),ComplexFrac(:),'b*exp(a/x)') %#ok<NOPRT>
            CoefficientValues=coeffvalues(f);
            [Q,R] = qr(o.Jacobian,0);
            Rinv = inv(R);
            CoeffErrors=sqrt(sum(Rinv.*Rinv,2)*g.rmse/g.dfe);
            ExpCoefficients=[CoefficientValues(:),CoeffErrors(:)];
            f=feval(f,TPU(1):0.01:TPU(end));
        else
            f=[]; % otherwise, neglect the fit
        end
        %..................................................................
        % 2D bar plot
        figure; bar(TPU,ComplexFrac,'facecolor','c'); hold on;
        if ~isempty(f)
    ...
    case 'Temperature and Linking Number'  % plot # of bound complexes VS T & LK
        ...
        if ~isempty(Filename) % if specified excel file
            ...
            if ~isempty(selected_sheets)
                ...
                xlabel('Linking number (LK)')
                zlabel('<Number of bound complexes> for 100 pits')
                % Heat map???

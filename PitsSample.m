...
%   GridRegistration function and follow the steps at the screen. Calls the
%   function CalculateBackground to estimate the background intensity
%   locally. Calls the function my_mask to collect the intensities
%   (relative,absolute,background) in the pits. my_mask normalizes the
%   background illumination. Sets the objects PitsChannel and
%   FRETefficiency and FRETanalysis.
%%                           PROPERTIES    
    properties
        Date=NaN; 
...
%========================= STORE VIDEOS OF PITS ===========================         
        function intensityMap=IntensityMap(Positions,Radius,...
            ...
            for i=1:numCols
                for j=1:numRows
                    x=max(0,round(Positions((i-1)*numRows+j,1)-Radius)):...
                        min(round(Positions((i-1)*numRows+j,1)+Radius),...
                        size(Data,2));
                    y=max(0,round(Positions((i-1)*numRows+j,2)-Radius)):...
...
function obj=CollectIntensitiesInPits(obj,Input)
            ...
            obj.Red_Channel_In_Green_Laser=PitsChannel(RIR,AIR,BIR);
            obj.Green_Channel_In_Green_Laser=PitsChannel(RIG,AIG,BIG);            
end
%==========================================================================
...
function obj=SavePitsVideos(obj,Input)
    try
        ...
        warning('Couldn''t save videos of pits.');
    end
end
%==========================================================================

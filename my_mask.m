...
function [Intensity,intensity,Background,binding] = ...
...
binding=cell2mat(cellfun(@(x) reshape(cellfun(@var,struct2cell(...
    regionprops(Pits,x,'PixelValues'))),[num_rows,num_cols]),...
    input,'UniformOutput',false));
binding=cell2mat(cellfun(@(x) medfilt1(x,w),...
    mat2cell(binding,ones(1,size(binding,1)),ones(1,size(binding,2)),...
    size(binding,3)),'UniformOutput',false));
binding = binding(:,:,1+w:size(binding,3)-w);

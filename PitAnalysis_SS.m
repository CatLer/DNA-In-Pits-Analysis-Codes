function meanPitInt = PitAnalysis(movieArray, varargin)
% This function takes a 3D movie array of a pit experiment and analyzes
% the intensity over time present in each pit. It comes equipped with a GUI 
% that allows the user to upload an image and choose where the pits can be
% found (either fringe image, or actual data). The user may also choose to
% save a movie with the pits drawn on it. This is done by declaring
% SaveMovie as a varargin. The varargin values MinInt and MaxInt can be
% inputted as well to change the max and min intensities of the image, if
% so desired.
...
if (~isempty(varargin))
    for k=1:numvarargin
        if strcmp(varargin{k}, 'SaveMovie')
            saveMovie = 1;
            saveName = [varargin{k+1} '.tif'];
        elseif strcmp(varargin{k}, 'MinInt')
            minInt = varargin{k+1}; 
...
while ~strcmp(pitHappy,'y') || strcmp(pitHappy, 'Y')
    ...
    if strcmp(otherPitFile, 'y') || strcmp(otherPitFile, 'Y')
        ...
        imshow(movieArray(:,:,1), [minInt maxInt]);
    end
...
% fits the pits on the image.
    ...
    hfig = figure; imshow(movieArray(:,:,1), []); hold on;
...
%% Save Movie with Pits
% This code only comes into play if the user wishes to save the movie with
% the pits drawn over it.
if saveMovie == 1 
	height = size(movieArray, 1);
	width = size(movieArray, 2);
    I = zeros(height, width, frames);
    for t = 1:frames
        f=imshow(movieArray(:,:,t), [minInt maxInt]);
        for i=1:(rowDist+1)
            for j=1:(colDist+1)
                line([rect(1,1)+(j-1)*pitColX+(i-1)*pitRowX  rect(1,1)+pitAvgX+(i-1)*pitRowX+(j-1)*pitColX],[rect(1,2)+(j-1)*pitColY+(i-1)*pitRowY rect(1,2)+(j-1)*pitColY+(i-1)*pitRowY], 'Color', 'r');
                line([rect(1,1)+(j-1)*pitColX+(i-1)*pitRowX  rect(1,1)+pitAvgX+(i-1)*pitRowX+(j-1)*pitColX],[rect(1,2)+(j-1)*pitColY+(i-1)*pitRowY+pitAvgY rect(1,2)+(j-1)*pitColY+(i-1)*pitRowY+pitAvgY], 'Color', 'r');
